...
let streamId = null; // To keep track of the first player's stream
...
io.on('connection', (socket) => {
    ...
    socket.on('video-stream', (data) => {
        ...
        // Broadcast to all clients, only if it's the first player
        if (socket.id === streamId) {
            socket.broadcast.emit('video-stream', data); // Send to all but the streamer
        }
    });
